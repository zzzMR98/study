...
Collator(bool wiping_dynamic_object);///构造函数
...
///
  ...
  ///
  void AddTrajectory(int trajectory_id,const std::unordered_set<string>& expected_sensor_ids, const Callback& callback);
///
...
///
 ...
 private:  
  OrderedMultiQueue queue_;  /// Queue keys are a pair of trajectory ID and sensor identifier. 
  std::unordered_map<int, std::vector<QueueKey>> queue_keys_; /// Map of trajectory ID to all associated QueueKeys.
