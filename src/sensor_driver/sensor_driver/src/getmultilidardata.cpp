...
struct GpsOption:public SensorOption
{
	GpsOption(){
		type = SensorOption::Type ::kGps;
	}
	int id = 0;
};
...
public:
  ...
  void cloud_callback (const CloudConstPtr& cloud,int id)
  {
	  ...
	  LOG(INFO)<<"id:"<<id;
	  CHECK_GT(cloud->size(),0);
	  cloudwithid->cloud = std::move(*cloud);
	//   std::vector<int> indices;
	//   pcl::removeNaNFromPointCloud(cloudwithid->cloud, cloudwithid->cloud, indices);
	  if(id%2==0)
	  {
...
//			LOG(INFO)<<"count:"<<count;
	...
	{
		ROS_INFO("get data from pcap: %s", pcap_file_name_.c_str());
		finish_read_data_thread_ =false;
...
//		    std::cout<<"offset"<<offset<<std::endl;
			...
			if(!started)
			{
				if(pcaptime < playback_start_at_*1000000)
					continue;
				started = true;
				playback_start_at_ = pcaptime-autualtime;
			}
			uSecDelay=pcaptime - autualtime - playback_start_at_ ;
...
//		LOG(INFO)<<"pcap end";
  ...
  /// \param len　数据长度
	void dispatchData(SensorOption* sensor,const unsigned char * data ,int len){
		...
		case SensorOption::Type::kGps:
		{
			unsigned  char* dstdata = new unsigned char[len];
			memcpy(dstdata,data,len);
			gpsdatas_.Push(std::make_shared<std::pair<int,unsigned char*>>(std::make_pair(len,dstdata)));
			break;
		}
		default :
			break;
	...
	void gpsprocess()
	{
		...
		while(!finish_read_data_thread_)
		{
			auto data = gpsdatas_.Pop();
			if(data == nullptr)
				continue;

			m_AnalysisGPS_.UpdateChar(data->first, data->second);
			delete [] data->second;
			...
			gps.gps.latitude = m_AnalysisGPS_.GPSData_struct.dLatitude;
			gps.gps.longitude = m_AnalysisGPS_.GPSData_struct.dLongitude;
			double heading_angle= m_AnalysisGPS_.GPSData_struct.PHDT_heading;
			if(heading_angle<=0)
			...
			gps.gps.altitude = m_AnalysisGPS_.GPSData_struct.Altitude;
			gps.roll = m_AnalysisGPS_.GPSData_struct.HDOP;
			gps.pitch = m_AnalysisGPS_.GPSData_struct.DGPSState;
			uint32_t gps_week = 0;
			uint64 gps_ms = 0;
            m_AnalysisGPS_.UTC2GPS(m_AnalysisGPS_.GPSData_struct.year, m_AnalysisGPS_.GPSData_struct.month, m_AnalysisGPS_.GPSData_struct.day,
                                   m_AnalysisGPS_.GPSData_struct.hour, m_AnalysisGPS_.GPSData_struct.min, m_AnalysisGPS_.GPSData_struct.second,
                                  &gps_week, &gps_ms);
            //LOG(ERROR) << "gps_week " << gps_week << " gps_ms " << gps_ms;
			pubGpsdata_.publish(gps);
...
//			LOG(INFO)<<"publish ins data";
  ...
  /// \param cloudwithid　点云
	...
	{
		XmlConf gpsconfig;
		if(!gpsconfig.Parse(configstr_.c_str(), "GetGPSData"))
		{
			std::cout<<"GetGPSData  is not exist in config xml  file"<<std::endl;
		}
		bool withgps = true;
		if(!gpsconfig.GetSystemParam("GetGPSData_on",withgps))
		{
		...
		if(withgps)
		{
			...
			GpsOption gpsoption;
			gpsoption.port = port;
			sensormaps_[port] = std::make_shared<GpsOption>(gpsoption);
			gps_read_data_thread_ = new boost::thread (boost::bind (&Lidardata::gpsprocess, this));
			pubGpsdata_ = nodehandle_.advertise<sensor_driver_msgs::GpswithHeading> ("GPSmsg", 50);
		}
  ...
  void ConfigParam()
  {
	...
	if(!xmlconfig_.GetSystemParam("playback_start_at",playback_start_at_))
	{
		playback_start_at_ = 0;
		std::cout<<"playback_start_at 不存在"<<std::endl;
	}
...
private:
  ...
  boost::thread* gps_read_data_thread_;//!<GPS data analysis thread
 ...
 ::ivcommon::BlockingQueue<std::shared_ptr<std::pair<int,unsigned char*>>> gpsdatas_;//!<GPS数据
