...
//			LOG(INFO)<<"publish ins data";
  ...
  bool init()
  {
	...
	for(int i=0;i<lidarnum_;i++)
	{
		...
		if(option.lasernum==32)
			intriparacalibration = "";//"S2_64db.xml";//
		else if(option.lasernum==64)
	//		  hdlCalibration_ =pkgPath+ "/config/S2_64db.xml";//
			intriparacalibration =pkgPath+ "/config/S2_64db.xml";//塔河比赛丰田
...
{
  ros::init(argc, argv, "getmultivelodynedata");
  ros::NodeHandle nh;
