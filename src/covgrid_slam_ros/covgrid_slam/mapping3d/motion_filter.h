...
#include "covgrid_slam/mapping3d/proto/motion_filter_options.pb.h"
...
class MotionFilter {
 public:
  ...
  // true is returned.
  bool IsSimilar(::ivcommon::Time time, const ivcommon::transform::Rigid3d& pose);

 private:
  const proto::MotionFilterOptions options_;
  int num_total_ = 0;
  int num_different_ = 0;
  ::ivcommon::Time last_time_;
  ivcommon::transform::Rigid3d last_pose_;
};
