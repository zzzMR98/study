...
#include "covgrid_slam/mapping3d/scan_matching/proto/fast_correlative_scan_matcher_options.pb.h"
#include "ivcommon/transform/transform.h"
#include "glog/logging.h"
#include "covgrid_slam/mapping3d/scan_matching/precomputation_grid.h"
...
FastCorrelativeScanMatcher::FastCorrelativeScanMatcher(
    ...
    : options_(options),
      ...
      precomputation_grid_stack_(
          ::ivcommon::make_unique<PrecomputationGridStack>(hybrid_grid, options)),
...
FastCorrelativeScanMatcher::FastCorrelativeScanMatcher(
    ...
    : options_(options),
      ...
      precomputation_grid_stack_(
          ::ivcommon::make_unique<PrecomputationGridStack>(hybrid_grid, options)),
      rotational_scan_matcher_(histogram),
	  scorecount_(0){}
...
bool FastCorrelativeScanMatcher::Match(
  ...
  const SearchParameters search_parameters{
      ::ivcommon::RoundToInt(options_.linear_xy_search_window() / resolution_),
      ::ivcommon::RoundToInt(options_.linear_z_search_window() / resolution_),
...
bool FastCorrelativeScanMatcher::MatchFullSubmap(
    ...
    ivcommon::transform::Rigid3d* const pose_estimate,
    float* const rotational_score) const {
  const ivcommon::transform::Rigid3d initial_pose_estimate(Eigen::Vector3d::Zero(),
                                                 gravity_alignment);
  double max_point_distance = 0.f;
  for (const Eigen::Vector3d& point : coarse_point_cloud) {
    max_point_distance = std::max(max_point_distance, point.norm());
  }
  const int linear_window_size =
      (width_in_voxels_ + 1) / 2 +
      ::ivcommon::RoundToInt(max_point_distance / resolution_ + 0.5f);
  const SearchParameters search_parameters{
...
bool FastCorrelativeScanMatcher::MatchWithSearchParameters(
    ...
    const sensor::PointCloud& coarse_point_cloud,
    const sensor::PointCloud& fine_point_cloud, const float min_score,
    float* const score, ivcommon::transform::Rigid3d* const pose_estimate,
    float* const rotational_score) const {
  CHECK_NOTNULL(score);
  ...
  LOG(WARNING)<<"discrete_scans:"<<discrete_scans.size();
...
DiscreteScan FastCorrelativeScanMatcher::DiscretizeScan(
    const FastCorrelativeScanMatcher::SearchParameters& search_parameters,
    const sensor::PointCloud& point_cloud, const ivcommon::transform::Rigid3d& pose,
...
std::vector<DiscreteScan> FastCorrelativeScanMatcher::GenerateDiscreteScans(
    ...
    const ivcommon::transform::Rigid3d& initial_pose) const {
  ...
  const float angular_step_size =
      kSafetyMargin * std::acos(1.f - ::ivcommon::Pow2(resolution_) /
                                          (2.f * ::ivcommon::Pow2(max_scan_range)));
  const int angular_window_size = ::ivcommon::RoundToInt(
  ...
  for (size_t i = 0; i != angles.size(); ++i) {
    ...
    const Eigen::Vector3d angle_axis(0.f, 0.f, angles[i]);
  ...
  if(result.size()>20)
  {
	  LOG(WARNING)<<"GenerateDiscreteScans:there are too many results,"<<result.size();
	  std::sort(result.begin(), result.end(), std::greater<DiscreteScan>());
	  result.erase(result.begin()+20,result.end());
  }
  return result;
}
...
FastCorrelativeScanMatcher::GenerateLowestResolutionCandidates(
  ...
  const int num_candidates =
      num_discrete_scans *
      ::ivcommon::Power(num_lowest_resolution_linear_xy_candidates, 2) *
...
ivcommon::transform::Rigid3d FastCorrelativeScanMatcher::GetPoseFromCandidate(
    const std::vector<DiscreteScan>& discrete_scans,
    const Candidate& candidate) const {
  return ivcommon::transform::Rigid3d::Translation(
             resolution_ * candidate.offset.matrix().cast<double>()) *
...
//    	  LOG(WARNING)<<"candidate.score1="<<candidate.score;
        return candidate;
      }
//      LOG(WARNING)<<"candidate.score2="<<candidate.score;
    }
    // All candidates have good scores but none passes the matching function.
    return best_high_resolution_candidate;
  }
//  LOG(WARNING)<<"candidates:"<<candidates.size()<<" depth:"<<candidate_depth;
  for (const Candidate& candidate : candidates) {
...
//    LOG(WARNING)<<"higher_resolution_candidates:"<<candidates.size()<<" depth:"<<candidate_depth-1;
...
//  LOG(WARNING)<<"best_high_resolution_candidate="<<best_high_resolution_candidate.score<<" candidate_depth="<<candidate_depth;
  return best_high_resolution_candidate;
}
