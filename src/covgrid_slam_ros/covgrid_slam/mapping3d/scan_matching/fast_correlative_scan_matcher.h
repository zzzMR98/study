...
#include "covgrid_slam/mapping/scan_matching/fast_correlative_scan_matcher.h"
...
struct DiscreteScan {
  ...
  bool operator<(const DiscreteScan& other) const { return rotational_score < other.rotational_score; }
  bool operator>(const DiscreteScan& other) const { return rotational_score > other.rotational_score; }
};
...
using MatchingFunction = std::function<bool(const ivcommon::transform::Rigid3d&)>;

/// \brief 搜索匹配，获得车辆相对于子地图的位姿
///
class FastCorrelativeScanMatcher {
 public:
  ...
  bool Match(const ivcommon::transform::Rigid3d& initial_pose_estimate,
             ...
             ivcommon::transform::Rigid3d* pose_estimate, float* rotational_score) const;
 ...
 private:
  ...
  ivcommon::transform::Rigid3d GetPoseFromCandidate(
      const std::vector<DiscreteScan>& discrete_scans,
