#ifndef COVGRID_MAPPING_3D_SCAN_MATCHING_INTERPOLATED_INTENSITY_GRID_H_
#define COVGRID_MAPPING_3D_SCAN_MATCHING_INTERPOLATED_INTENSITY_GRID_H_
...
class InterpolatedIntensityGrid {
 public:
  explicit InterpolatedIntensityGrid(const IntensityHybridGrid& hybrid_grid)
      : hybrid_grid_(hybrid_grid) {}

  InterpolatedIntensityGrid(const InterpolatedIntensityGrid&) = delete;
  InterpolatedIntensityGrid& operator=(const InterpolatedIntensityGrid&) = delete;
  ...
  template <typename T>
  T GetValue(const T& x, const T& y, const T& z) const {
    double x1, y1, z1, x2, y2, z2;
    ComputeInterpolationDataPoints(x, y, z, &x1, &y1, &z1, &x2, &y2, &z2);//1,2 分别代表

    const Eigen::Array3i index1 =
        hybrid_grid_.GetCellIndex(Eigen::Vector3d(x1, y1, z1));
    const double q111 = hybrid_grid_.GetAverage(index1);
    const double q112 =
        hybrid_grid_.GetAverage(index1 + Eigen::Array3i(0, 0, 1));
    const double q121 =
        hybrid_grid_.GetAverage(index1 + Eigen::Array3i(0, 1, 0));
    const double q122 =
        hybrid_grid_.GetAverage(index1 + Eigen::Array3i(0, 1, 1));
    const double q211 =
        hybrid_grid_.GetAverage(index1 + Eigen::Array3i(1, 0, 0));
    const double q212 =
        hybrid_grid_.GetAverage(index1 + Eigen::Array3i(1, 0, 1));
    const double q221 =
        hybrid_grid_.GetAverage(index1 + Eigen::Array3i(1, 1, 0));
    const double q222 =
        hybrid_grid_.GetAverage(index1 + Eigen::Array3i(1, 1, 1));
 ...
 private:
  ...
  Eigen::Vector3d CenterOfLowerVoxel(const double x, const double y,
    ...
    Eigen::Vector3d center = hybrid_grid_.GetCenterOfCell(
        hybrid_grid_.GetCellIndex(Eigen::Vector3d(x, y, z)));
  ...
  const IntensityHybridGrid& hybrid_grid_;
};

}  // namespace scan_matching
}  // namespace mapping_3d

#endif  // COVGRID_MAPPING_3D_SCAN_MATCHING_INTERPOLATED_INTENSITY_GRID_H_
