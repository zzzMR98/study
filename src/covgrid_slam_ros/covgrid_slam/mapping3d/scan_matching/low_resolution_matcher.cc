...
float EvaluateLowResolutionScore(const FeatureHybridGrid& low_resolution_grid,
...
std::function<bool(const ivcommon::transform::Rigid3d&)> CreateLowResolutionMatcher(
    const FeatureHybridGrid* low_resolution_grid, const sensor::PointCloud* points,
    const float min_low_resolution_score) {
  return [=](const ivcommon::transform::Rigid3d& pose) {
