...
PrecomputationGrid ConvertToPrecomputationGrid(const FeatureHybridGrid& hybrid_grid) {
  PrecomputationGrid result(hybrid_grid.resolution());
  for (auto it = FeatureHybridGrid::Iterator(hybrid_grid); !it.Done(); it.Next()) {
    const int cell_value = ::ivcommon::RoundToInt(
        (mapping::ValueToProbability(it.GetValue().probability) -
