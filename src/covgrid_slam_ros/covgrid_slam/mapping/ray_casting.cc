...
#include "covgrid_slam/mapping/ray_casting.h"
...
void GrowAsNeeded(const sensor::RangeData& range_data,
  ...
  for (const Eigen::Vector3d& hit : range_data.returns) {
    bounding_box.extend(hit.head<2>());
  }
  for (const Eigen::Vector3d& miss : range_data.misses) {
...
void CastRays(const sensor::RangeData& range_data,
  ...
  for (const Eigen::Vector3d& missing_echo : range_data.misses) {
