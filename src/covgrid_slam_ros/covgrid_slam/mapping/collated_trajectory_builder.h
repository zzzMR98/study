...
#include "covgrid_slam/mapping/trajectory_builder.h"
#include "covgrid_slam/mapping3d/local_trajectory_builder.h"
#include "covgrid_slam/sensor/collator.h"
...
class CollatedTrajectoryBuilder : public TrajectoryBuilder {
 ...
 private:
  ...
  std::unique_ptr<mapping3d::LocalTrajectoryBuilder> wrapped_trajectory_builder_;

  // Time at which we last logged the rates of incoming sensor data.
  std::chrono::steady_clock::time_point last_logging_time_;
  std::map<string, ::ivcommon::RateTimer<>> rate_timers_;
};
