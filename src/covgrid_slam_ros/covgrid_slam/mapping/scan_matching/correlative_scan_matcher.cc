...
SearchParameters::SearchParameters(const double linear_search_window,
  ...
  angular_perturbation_step_size =
      kSafetyMargin * std::acos(1. - ::ivcommon::Pow2(resolution) /
                                         (2. * ::ivcommon::Pow2(max_scan_range)));
...
std::vector<sensor::PointCloud> GenerateRotatedScans(
  ...
  for (int scan_index = 0; scan_index < search_parameters.num_scans;
    ...
    rotated_scans.push_back(sensor::TransformPointCloud(
        point_cloud, ivcommon::transform::Rigid3d::Rotation(Eigen::AngleAxisd(
  ...
  for (const sensor::PointCloud& scan : scans) {
    ...
    for (const Eigen::Vector3d& point : scan) {
      const Eigen::Vector2d translated_point =
          Eigen::Affine2d(initial_translation) * point.head<2>();
