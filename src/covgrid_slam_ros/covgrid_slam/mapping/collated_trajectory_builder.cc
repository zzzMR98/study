...
#include "covgrid_slam/mapping/collated_trajectory_builder.h"
...
CollatedTrajectoryBuilder::CollatedTrajectoryBuilder(
    ...
    std::unique_ptr<mapping3d::LocalTrajectoryBuilder>
  ...
  sensor_collator_->AddTrajectory(
      ...
      [this](const string& sensor_id, std::unique_ptr<sensor::Data> data) {
        HandleCollatedSensorData(sensor_id, std::move(data));   //callback function for sensor data
      });
}
...
void CollatedTrajectoryBuilder::HandleCollatedSensorData(
    const string& sensor_id, std::unique_ptr<sensor::Data> data) {
  //LOG(INFO)<<"cartographer is running the callback HandleCollatedSensorData(),it is going to process sensor_data!";
    ...
    it = rate_timers_
             .emplace(
                 ...
                 std::forward_as_tuple(
                     ::ivcommon::FromSeconds(kSensorDataRatesLoggingPeriodSeconds)))
             .first;
  }
