...
PointCloud AdaptivelyIntensityVoxelFiltered(
    const proto::AdaptiveVoxelFilterOptions& options,
    const PointCloud& point_cloud) {
  if (point_cloud.size() <= options.min_num_points()) {
    // 'point_cloud' is already sparse enough.
...
//	point_cloud_.reserve(point_cloud.size());
  for (const auto& point : point_cloud) {
...
MaxIntensityVoxelFilter::MaxIntensityVoxelFilter(const float size) : max_intensity_voxels_(size) {}

void MaxIntensityVoxelFilter::InsertPointCloud(const PointCloud& point_cloud) {
//	point_cloud_.reserve(point_cloud.size());
  for ( int i=0;i<point_cloud.size();i++) {
	  const auto& point = point_cloud[i];
    auto* const value = max_intensity_voxels_.mutable_value(max_intensity_voxels_.GetCellIndex(point));
    if (value->second <= point.intensity) {
      value->second = point.intensity;
      value->first = i;
    }
  }
  point_cloud_.reserve(point_cloud.size());
  for(auto voxel:max_intensity_voxels_)
  {
	  if(voxel.second.second>0.5)
		  point_cloud_.push_back(point_cloud[voxel.second.first]);
  }
}


const PointCloud& MaxIntensityVoxelFilter::point_cloud() const { return point_cloud_; }
...
PointCloud AdaptiveVoxelFilter::MaxIntensityFilter(const PointCloud& point_cloud) const {
  return AdaptivelyIntensityVoxelFiltered(
      options_, FilterByMaxRange(point_cloud, options_.max_range()));
}
}  // namespace sensor
