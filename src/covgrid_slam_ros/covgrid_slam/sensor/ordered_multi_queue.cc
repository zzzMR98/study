...
#include "covgrid_slam/sensor/ordered_multi_queue.h"
...
void OrderedMultiQueue::Dispatch() {
  while (true) {
    ...
    for (auto it = queues_.begin(); it != queues_.end();) {
      const auto* data = it->second.queue.Peek<Data>();//return deque_.front()lzz
      if (data == nullptr) {
    ...
    const ::ivcommon::Time common_start_time =
        GetCommonStartTime(next_queue_key.trajectory_id);//return the time when we got the sensor data lzz

    if (next_data->time >= common_start_time) {
      // Happy case, we are beyond the 'common_start_time' already.
      last_dispatched_time_ = next_data->time;
      next_queue->callback(next_queue->queue.Pop());// return deque_.front() lzz
