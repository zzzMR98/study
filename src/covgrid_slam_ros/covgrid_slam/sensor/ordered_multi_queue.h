...
class OrderedMultiQueue {
 ...
 private:
  ...
  ::ivcommon::Time GetCommonStartTime(int trajectory_id);

  // Used to verify that values are dispatched in sorted order.
  ::ivcommon::Time last_dispatched_time_ = ::ivcommon::Time::min();

  std::map<int, ::ivcommon::Time> common_start_time_per_trajectory_;
