...
Eigen::Vector3d CompressedPointCloud::ConstIterator::operator*() const {
...
CompressedPointCloud::CompressedPointCloud(const PointCloud& point_cloud)
  ...
  using Blocks = mapping3d::HybridGridBase<std::vector<RasterPoint>>;
  ...
  for (int point_index = 0; point_index < static_cast<int>(point_cloud.size());
       ++point_index) {
    const Eigen::Vector3d& point = point_cloud[point_index];
...
PointCloud CompressedPointCloud::Decompress() const {
  ...
  for (const Eigen::Vector3d& point : *this) {
