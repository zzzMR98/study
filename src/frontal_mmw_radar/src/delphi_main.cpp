...
class PostProcess {
public:
	PostProcess(ros::NodeHandle& nodehandle) :
			nodehandle_(nodehandle),
			//      image_transport_nh_(nodehandle), //for transport image
			processthread_(NULL), processthreadfinished_(false) {
	...
	void init() {
		subRadarData_ = nodehandle_.subscribe<frontal_mmw_radar_msgs::RadarData>(
				"radardata", 1,
				boost::bind(&PostProcess::RadarDataHandler, this, _1));

		processthread_ = new boost::thread(
				boost::bind(&PostProcess::process, this));
	}

	void RadarDataHandler(
			const frontal_mmw_radar_msgs::RadarDataConstPtr& radar_msg) {

//    ROS_INFO("<main> radar data callback...");
		for (int i = 0; i < 64; ++i) {
			radar_data_.delphi_detection_array[i].target_ID =
					radar_msg->delphi_detection_array[i].target_ID;
			radar_data_.delphi_detection_array[i].range =
					radar_msg->delphi_detection_array[i].range;
			radar_data_.delphi_detection_array[i].v =
					radar_msg->delphi_detection_array[i].v;
			radar_data_.delphi_detection_array[i].x =
					radar_msg->delphi_detection_array[i].x;
			radar_data_.delphi_detection_array[i].y =
					radar_msg->delphi_detection_array[i].y;
			radar_data_.delphi_detection_array[i].angle =
					radar_msg->delphi_detection_array[i].angle;
			radar_data_.delphi_detection_array[i].valid =
					radar_msg->delphi_detection_array[i].valid;
			radar_data_.delphi_detection_array[i].status =
					radar_msg->delphi_detection_array[i].status;
			radar_data_.delphi_detection_array[i].moving =
					radar_msg->delphi_detection_array[i].moving;
			radar_data_.delphi_detection_array[i].moving_fast =
					radar_msg->delphi_detection_array[i].moving_fast;
			radar_data_.delphi_detection_array[i].moving_slow =
					radar_msg->delphi_detection_array[i].moving_slow;
		}
