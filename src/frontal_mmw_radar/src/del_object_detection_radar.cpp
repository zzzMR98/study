...
ObjectDetection::ObjectDetection(void)
{
    ...
    m_MMW_img_compare = cvCreateImage(cvSize(PLANE_WIDTH, PLANE_HEIGHT), IPL_DEPTH_8U, 3);
    cvZero(m_MMW_img_bak);
    cvZero(m_MMW_img);
    cvZero(m_MMW_img_compare);
    ...
    for(int i = 0; i<NUM; i++)
    {
    	...
    	MMW_detection_array[i].x = 0.0;
    	MMW_detection_array[i].y = 0.0;
    	MMW_detection_array[i].v = 0.0;
    	MMW_detection_array[i].angle = 0.0;
    	MMW_detection_array[i].range = 0.0;
    	MMW_detection_array[i].valid = 0;
    	MMW_detection_array[i].moving = 0;
    	MMW_detection_array[i].moving_fast =0;
    	MMW_detection_array[i].moving_slow = 0;
    	MMW_detection_array[i].status =0;
...
void ObjectDetection::set_radar_data(const delphi_radar_target& radar)
{
  ...
  vehicle_speed_origin_ = radar.vehicle_speed_origin; //原始ECU车速
    ...
    for (int i = 0; i < NUM; ++i)
    {
    	MMW_detection_array[i].target_ID= radar.delphi_detection_array[i].target_ID;
    	MMW_detection_array[i].x = radar.delphi_detection_array[i].x;
    	MMW_detection_array[i].y = radar.delphi_detection_array[i].y;
    	MMW_detection_array[i].v = radar.delphi_detection_array[i].v;
    	MMW_detection_array[i].range = radar.delphi_detection_array[i].range;
    	MMW_detection_array[i].angle = radar.delphi_detection_array[i].angle;
    	MMW_detection_array[i].valid = radar.delphi_detection_array[i].valid;
    	MMW_detection_array[i].moving = radar.delphi_detection_array[i].moving;
    	MMW_detection_array[i].moving_fast = radar.delphi_detection_array[i].moving_fast;
    	MMW_detection_array[i].moving_slow = radar.delphi_detection_array[i].moving_slow;
    	MMW_detection_array[i].status = radar.delphi_detection_array[i].status;
...
void ObjectDetection::set_radar_Data(delphi_radar_target& radar,
	...
	for (int i = 0; i < NUM; ++i) {
		MMW_detection_array[i].target_ID =
				radar.delphi_detection_array[i].target_ID;
		MMW_detection_array[i].x = radar.delphi_detection_array[i].x;
		MMW_detection_array[i].y = radar.delphi_detection_array[i].y;
		MMW_detection_array[i].v = radar.delphi_detection_array[i].v;
		...
		MMW_detection_array[i].angle = radar.delphi_detection_array[i].angle;
		MMW_detection_array[i].valid = radar.delphi_detection_array[i].valid;
		MMW_detection_array[i].moving =
				radar.delphi_detection_array[i].moving;
		MMW_detection_array[i].moving_fast =
				radar.delphi_detection_array[i].moving_fast;
		MMW_detection_array[i].moving_slow =
				radar.delphi_detection_array[i].moving_slow;
		MMW_detection_array[i].status =
				radar.delphi_detection_array[i].status;
...
void ObjectDetection::main_function2()
{
    ...
    for(int i = 0;i<NUM;i++)
    {
        if((MMW_detection_array[i].range>1.0))//径向距离大于1m才有效
        {
...
void ObjectDetection::compare()
{
    ...
    for(int i = 0;i<NUM;i++)
    {
        if(MMW_detection_array[i].status!=0)
        {
            //筛选得到由XLim范围限制的前方车辆
            vecObj_Interest.push_back(MMW_detection_array[i]);
        }
    }
    ...
    if(!vecObj_Interest.empty()) //有毫米波检测目标
    {
        ...
        {
            final_obj = MMW_detection_array[m_ACC_ID-1];
        }
    }
...
void ObjectDetection::show_result(moving_object_millimeter& obj_interest) //显示ACC目标
{
    ...
    //鸟瞰图中显示红色点
    cvCircle(m_MMW_img,delphi_pos, 1, cvScalar(0, 0, 255), 6);
    //ACC target ID
    sprintf(text,"ACC: %d",obj_interest.target_ID);
    cvPutText(m_MMW_img,text,cvPoint(160,15),&cf,cvScalar(0,0,255));//red color
...
//      printf("<object_delphi_radar> enter in show_result...\n");
        //毫米波检测目标点在鸟瞰图上的坐标
      MMW_pos.x = persMap_Middle + (*it).x*METER2PIXEL;
      MMW_pos.y = (PLANE_HEIGHT - 1) - ((*it).y + RADAR2CAR)*METER2PIXEL;
        //鸟瞰图中显示白色点
        ...
        /*************绘制另一幅对比图********************/
        cvCircle(m_MMW_img_compare,MMW_pos, 1, cvScalar(255,255, 255), 2);
        ...
        cvPutText(m_MMW_img_compare,text,cvPoint(MMW_pos.x+2,MMW_pos.y),&cf,cvScalar(0,255,255));
        /*************绘制另一幅对比图********************/
...
void ObjectDetection::ShowACCTarget(const int& ACC_ID)
{
  if(ACC_ID != 0)//has ACC target
  {
    CvPoint delphi_pos;
    //横坐标等于中点,只显示纵坐标
    delphi_pos.x = persMap_Middle+ MMW_detection_array[ACC_ID-1].x*METER2PIXEL;
    delphi_pos.y = (PLANE_HEIGHT - 1) - (MMW_detection_array[ACC_ID-1].y + RADAR2CAR)*METER2PIXEL;

    //鸟瞰图中显示红色点
    cvCircle(m_MMW_img,delphi_pos, 1, cvScalar(0, 0, 255), 6);
  }
  sprintf(text,"ACC: %d",ACC_ID);
  cvPutText(m_MMW_img,text,cvPoint(350,15),&cf,cvScalar(0,0,255));//red color
...
void ObjectDetection::DrawCommonElements()
{
  ...
  //显示其他信息
   ...
   cvPutText(m_MMW_img,text,cvPoint(5,15),&cf,cvScalar(0,0,255));
   sprintf(text,"ego vehicle speed origin: %.3f",vehicle_speed_origin_);//ECU实际车速
   cvPutText(m_MMW_img,text,cvPoint(5,25),&cf,cvScalar(0,0,255));
   cvCopy(m_MMW_img,m_MMW_img_compare);//基本信息复制一份给另一幅图像
...
void ObjectDetection::DisplayAll()
{
  ...
  cvNamedWindow("MMW_image_compare",CV_WINDOW_NORMAL);
  cvShowImage("MMW_image_compare", m_MMW_img_compare);
  cvWaitKey(10);
}
