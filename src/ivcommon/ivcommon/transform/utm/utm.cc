...
state_struct Position_Trans_From_ECEF_To_UTM(double latitude,double longitude,double e0, double n0)
{
	...
	double	A	 = (double)(( longitude - lonBase )*M_PI/180.0 * cos(latitude*M_PI/180.0));		// ���ն���������
	double	sPhi = (double)((1 - pow(WGS84_ECCENTRICITY,2)/4.0 - 3*pow(WGS84_ECCENTRICITY,4)/64.0 - 5*pow(WGS84_ECCENTRICITY,6)/256.0) * latitude*M_PI/180.0
					- (3*pow(WGS84_ECCENTRICITY,2)/8.0 + 3*pow(WGS84_ECCENTRICITY,4)/32.0 + 45*pow(WGS84_ECCENTRICITY,6)/1024.0) * sin(2*latitude*M_PI/180.0)
					+ (15*pow(WGS84_ECCENTRICITY,4)/256.0 + 45*pow(WGS84_ECCENTRICITY,6)/256.0)* sin(4*latitude*M_PI/180.0)
					- (35*pow(WGS84_ECCENTRICITY,6)/3072.0) * sin(6*latitude*M_PI/180.0));
	double	T	= (double)(pow(tan(latitude*M_PI/180.0),2));
	double	C	= (double)((pow(WGS84_ECCENTRICITY,2)/(1 - pow(WGS84_ECCENTRICITY,2))) * pow(cos(latitude*M_PI/180.0),2));

	result.position.x = (double)((k0*WGS84_EQUATORIAL_RADIUS*vPhi*(A + (1 - T + C)*pow(A,3)/6.0 
				+ (5 - 18*T + pow(T,2))*pow(A,5)/120.0))*1000 - e0);
	result.position.y = (double)((k0*WGS84_EQUATORIAL_RADIUS*(sPhi + vPhi * tan(latitude*M_PI/180.0)*(pow(A,2)/2
...
/// \param N 北向距离/m
/// \param E 东向距离/m
/// \param lat_rad　纬度　弧度　输出
/// \param lon_rad　经度　弧度　输出
