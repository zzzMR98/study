...
class Rigid2 {
 public:
  ...
  double normalized_angle() const {
    return ::ivcommon::NormalizeAngleDifference(rotation().angle());
  }
...
class Rigid3 {
 public:
  using Vector = Eigen::Matrix<FloatType, 3, 1>;//平移向量（3X1）
  using Quaternion = Eigen::Quaternion<FloatType>;//四元数（4X1）
  using AngleAxis = Eigen::AngleAxis<FloatType>;//旋转向量（3X1）该向量的方向就是旋转轴，它的模就是绕轴逆时针旋转的角度
...
template <typename T>
void operator>>( std::fstream& os,
                         transform::Rigid3<T>& rigid) {
  T translation_x(0) ,translation_y(0),translation_z(0);
  T quarternion_w(0),quarternion_x(0),quarternion_y(0),quarternion_z(0);  
  
//   if(os.eof()){std::cerr<< "the file pending to read ended unnormally!";}
  os>>translation_x>>translation_y>>translation_z>>quarternion_w>>quarternion_x>>quarternion_y>>quarternion_z;
  rigid = transform::Rigid3<T>{Eigen::Matrix<T, 3, 1>(translation_x,translation_y,translation_z),
    Eigen::Quaternion<T>(quarternion_w,quarternion_x,quarternion_y,quarternion_z)};

}

// template <typename T>
// void operator<<(std::fstream& os,const transform::Rigid3<T>& rigid) {
//   os << rigid.translation().x()<<'\t'<<rigid.translation().y()<<'\t'<<rigid.translation().z()<<'\t'<<rigid.rotation().w()
...
// Rigid3d FromDictionary(::ivcommon::LuaParameterDictionary* dictionary);



struct posestamped{
  posestamped() = default;
  posestamped(::ivcommon::Time time, const transform::Rigid3d& pose)
  :time(time)
  ,pose(pose){}
