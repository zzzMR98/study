...
Eigen::Matrix<T, 3, 1> RotationQuaternionToAngleAxisVector(
  ...
  const T angle = T(2.) * std::atan2(normalized_quaternion.vec().norm(),
...
Rigid2d ToRigid2(const proto::Rigid2d& transform);/// Conversions between Rigid2d and proto.
Eigen::Vector2d ToEigen(const proto::Vector2d& vector);/// Conversions between Eigen::Vector2d and proto.
Eigen::Vector3f ToEigen(const proto::Vector3f& vector);/// Conversions between Eigen::Vector3f and proto.
Eigen::Vector3d ToEigen(const proto::Vector3d& vector);/// Conversions between Eigen::Vector3d and proto.
Eigen::Quaterniond ToEigen(const proto::Quaterniond& quaternion);/// Conversions between Eigen::Quaterniond and proto.
proto::Rigid2d ToProto(const Rigid2d& transform);/// Conversions between Rigid2d and proto.
proto::Rigid2f ToProto(const Rigid2f& transform);/// Conversions between Rigid2f and proto.
proto::Rigid3d ToProto(const Rigid3d& rigid);/// Conversions between Rigid3d and proto.
Rigid3d ToRigid3(const proto::Rigid3d& rigid);/// Conversions between ToRigid3 and proto.
proto::Rigid3f ToProto(const Rigid3f& rigid);/// Conversions between Rigid3f and proto.
proto::Vector2d ToProto(const Eigen::Vector2d& vector);/// Conversions between Eigen::Vector2d and proto.
proto::Vector3f ToProto(const Eigen::Vector3f& vector);/// Conversions between Eigen::Vector2f and proto.
proto::Vector3d ToProto(const Eigen::Vector3d& vector);/// Conversions between Eigen::Vector3d and proto.
proto::Quaternionf ToProto(const Eigen::Quaternionf& quaternion);/// Conversions between Eigen::Quaternionf and proto.
proto::Quaterniond ToProto(const Eigen::Quaterniond& quaternion);/// Conversions between Eigen::Quaterniond and proto.
