...
class BlockingQueue {
 public:
  ...
  bool PushWithTimeout(T t, const ivcommon::Duration timeout) {
    MutexLocker lock(&mutex_);
  ...
  ///
  /// \param timeout　阻塞最长时间
  T PopWithTimeout(const ivcommon::Duration timeout) {
    MutexLocker lock(&mutex_);
    if (!lock.AwaitWithTimeout(
 ...
 private:
  ...
  std::deque<T> deque_ GUARDED_BY(mutex_); //双端队列
