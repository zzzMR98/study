...
class ConfigurationFileResolver : public FileResolver {
 public:
  explicit ConfigurationFileResolver(
      const std::vector<string>& configuration_files_directories); ///设置目录

  string GetFullPathOrDie(const string& basename) override;///获取文件完整路径
  string GetFileContentOrDie(const string& basename) override;///获取文件完整内容
