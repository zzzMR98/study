...
class RateTimer {
 public:
  ...
  // time.
  explicit RateTimer(const ivcommon::Duration window_duration)
  ...
  double ComputeRate() const {
    ...
    return static_cast<double>(events_.size() - 1) /
           ivcommon::ToSeconds((events_.back().time - events_.front().time));
  }
  ...
  double ComputeWallTimeRateRatio() const {
    ...
    return ivcommon::ToSeconds((events_.back().time - events_.front().time)) /
 ...
 private:
  ...
  std::vector<double> ComputeDeltasInSeconds() const {
    ...
    for (size_t i = 0; i != count; ++i) {
      result.push_back(
          ivcommon::ToSeconds(events_[i + 1].time - events_[i].time));
    }
    return result;
  }
  ...
  string DeltasDebugString() const {
    ...
    for (const double x : deltas) {
      squared_sum += ivcommon::Pow2(x - mean);
    }
  ...
  const ivcommon::Duration window_duration_;
};
