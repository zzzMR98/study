  ...
  class SCOPED_CAPABILITY Locker {
   public:
    ...
    void Await(Predicate predicate) REQUIRES(this) {
      mutex_->condition_.wait(lock_, predicate);///当predicate为ｆａｌｓｅ时才会阻塞线程，当predicate为ｆａｌｓｅ时会一直阻塞直到状态变量唤醒
    }

    template <typename Predicate>
    bool AwaitWithTimeout(Predicate predicate, ivcommon::Duration timeout)
        REQUIRES(this) {
      return mutex_->condition_.wait_for(lock_, timeout, predicate);///当predicate为ｆａｌｓｅ时才会阻塞线程，
					///当predicate为ｆａｌｓｅ时会一直阻塞直到状态变量唤醒或者阻塞超过了一定时间
    }

   private:
    Mutex* mutex_;
    std::unique_lock<std::mutex> lock_;//锁管理模板类,在unique_lock对象的声明周期内，它所管理的锁对象会一直保持上锁状态
  };

 private:
  std::condition_variable condition_;///状态变量
  std::mutex mutex_;
};
